...
sidebar_position: 3
slug: /getting-started/usage/simple
---
...
```tsx
import { ViewModelProps, ViewModeBase, withViewModel } from 'mobx-view-model';

export class MyPageVM extends ViewModeBase<{ payloadA: string }> {
  @observable
  accessor state = '';
...
const MyPageView = observer(({ payload }: { payload: ViewModelPayload<MyPageVM> }) => {
