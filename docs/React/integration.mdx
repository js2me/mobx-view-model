...
## 3. [Optional] Using [ViewModelStore](/api/view-model-store/interface)  
...
To use this store:
  ...
  2. Wrap your application into [`ViewModelsProvider`](/react/api/view-models-provide) Context Provider.  
...
:::note [`isMounted`](/api/view-models/interface#ismounted) state  
This statement totally based on call [`mount()` method](/api/view-models/interface#mount) which calls inside [`useCreateViewModel()`](/react/api/use-create-view-model) hook.  
...
:::warning Do not calls [`mount()`](/api/view-models/interface#mount), [`unmount()`](/api/view-models/interface#unmount) manually  
