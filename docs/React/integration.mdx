...
:::note [`isMounted`](/api/view-models/interface#ismounted-boolean) state  
This statement totally based on call [`mount()` method](/api/view-models/interface#mount-void--promisevoid) which calls inside [`useCreateViewModel()`](/react/api/use-create-view-model) hook.  
...
:::warning Do not calls [`mount()`](/api/view-models/interface#mount-void--promisevoid), [`unmount()`](/api/view-models/interface#unmount-void--promisevoid) manually  
