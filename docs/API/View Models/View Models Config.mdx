...
- `'strict'` - **(default)** strict equality ([lodash isEqual](https://lodash.com/docs/4.17.15#isEqual))  
...
- `'struct'` - [`computed.struct` from MobX](https://mobx.js.org/computeds.html#computed-struct)  
...
- `false` - **(default)** do not wrap `payload` into `computed` MobX utility  
