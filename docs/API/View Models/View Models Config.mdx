...
import { circularVmPayloadDependencyTestCases } from '@site/../src/hoc/with-view-model.test.fixture.ts';
...
- `'strict'` - _**(default)**_ strict equality ([lodash isEqual](https://lodash.com/docs/4.17.15#isEqual))  
...
{circularVmPayloadDependencyTestCases.filter(it => it.isRecursion).map(it => (
  <>
    <CodeBlock language="ts">{JSON.stringify(it.vmConfig, null, 2)}</CodeBlock>
  </>
))}